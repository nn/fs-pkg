...
void *db_query(enum db_query_res_type type, const char *fmt, ...) {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
       ...
       Log(LOG_DEBUG, "Got %d columns", cols);
...
*/
          switch (type) {
             ...
             case QUERY_CHAR:
                ...
                break;
             case QUERY_INODE:
                ret = (void *)blockheap_alloc(vfs_inode_heap);
                break;
             default:
                break;
          }
...
int db_sqlite_open(const char *path) {
   ...
   /* If the database doesnt exist or is memory-backed, initialize it */
   if (!exists)
...
int db_pkg_add(const char *path) {
    ...
    db_pkg_remove(path);
    db_query(QUERY_NULL, "INSERT INTO packages (path, version) VALUES ('%s', %lu);", path, sb.st_mtime);
    return (u_int32_t)db_query(QUERY_INT, "SELECT id FROM packages WHERE path = '%s' AND version = %lu;", path, sb.st_mtime);
}
...
int db_file_remove(int pkg, const char *path) {
    ...
    db_query(QUERY_NULL, "DELETE FROM files WHERE package = '%lu' AND path = '%s';",
             pkg, path);
    return EXIT_SUCCESS;
}
